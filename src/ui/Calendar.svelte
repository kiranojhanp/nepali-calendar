...
<script lang="ts">
	...
	$: if (displayedMonth) {
		...
		if (
			nepali.year !== currentMonthNepali.year ||
			nepali.month !== currentMonthNepali.month
		) {
			currentMonthNepali = {
				year: nepali.year,
				month: nepali.month,
	...
	$: {
		...
		if ($settings.highlightHolidays) {
			calendarDays = calendarDays.map((day) => {
				// Check if day is from API holidays
				const isAPIHoliday =
					day.isCurrentMonth &&
					holidayMap.has(
						`${day.nepali.year}-${day.nepali.month}-${day.nepali.day}`,
					);

				// Check if day is a weekend based on settings
				const dayOfWeek = day.gregorian.day(); // 0=Sunday, 1=Monday, ..., 6=Saturday
				const isWeekend =
				...
				return {
					...day,
					hasHoliday:
						day.isCurrentMonth && (isAPIHoliday || isWeekend),
				};
			});
		}
			...
			if (Array.isArray(data)) {
				data.forEach((dayData) => {
					...
					if (
						...
						Array.isArray(dayData.eventDetails)
					) {
						const hasHoliday = dayData.eventDetails.some(
...
<div id="calendar-container">
	<nav>
		<div class="left-nav">
			<div class="title">
				<span class="month"
					>{np_nepaliMonths[currentMonthNepali.month - 1]}</span
				>
				<span class="year"
					>{nepaliNumber(String(currentMonthNepali.year))}</span
				>
			</div>
		</div>
		<div class="right-nav">
			...
			<button
				class="reset-button"
				on:click={goToToday}
				aria-label={TOOLTIP_TODAY}>TODAY</button
			>
			<div
				class="nav-button"
				on:click={nextMonth}
					...
					{#each calendarDays.slice(row * 7, (row + 1) * 7) as day}
						<td>
							<div
								...
								on:mouseenter={(e) =>
									onHoverDay &&
									onHoverDay(day.gregorian, e.target)}
								on:contextmenu={(e) =>
									onContextMenuDay &&
									onContextMenuDay(day.gregorian, e)}
							>
